...
def test_pyproject_toml_exists():
    ...
    assert "[tool.ruff.lint]" in content, (
        "pyproject.toml must contain [tool.ruff.lint] section"
    )
    assert "[tool.ruff.format]" in content, (
        "pyproject.toml must contain [tool.ruff.format] section"
    )

    # Check for specific configuration values
    assert "line-length = 88" in content, (
        "Ruff must be configured with line-length = 88"
    )
...
def test_pre_commit_config_exists():
    ...
    assert "pre-commit/pre-commit-hooks" in content, (
        "Must include standard pre-commit hooks"
    )
    ...
    assert "--exit-non-zero-on-fix" in content, (
        "Ruff hook must include --exit-non-zero-on-fix argument"
    )
...
"""
    ...
    try:
        ...
        result = subprocess.run(
            ["ruff", "check", temp_file], cwd=repo_root, capture_output=True, text=True
        )
        ...
        result = subprocess.run(
            ["ruff", "format", temp_file], cwd=repo_root, capture_output=True, text=True
        )
        ...
        assert "def test(x, y):" in formatted_code, (
            "Function signature should be properly formatted"
        )
...
"""
    ...
    try:
        ...
        assert result.returncode == 0, (
            f"Ruff check should pass with our configuration. Output: {result.stdout}"
        )
