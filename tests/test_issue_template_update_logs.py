...
def test_update_logs_template_has_correct_frontmatter():
    ...
    assert "about: Use this template to log AI conversations and update key documents." in content, "Template must have correct about description"
    assert "title: 'LOG: [Brief description of changes]'" in content, "Template must have correct default title"
...
def test_update_logs_template_has_correct_instructions():
    ...
    assert "development session with the AI" in content, "Template must mention AI development sessions"
    assert "documentation/prompt_log.md" in content, "Template must reference documentation/prompt_log.md"
    ...
    assert "(Paste AI conversation here)" in content, "Template must have placeholder for AI conversation"
    assert "(Paste any identified technical debt here, or \"N/A\" if none)" in content, "Template must have placeholder for technical debt"
    assert "(Paste new content for current_project_state.md here, or \"N/A\" if no architectural changes)" in content, "Template must have placeholder for project state"
...
def test_update_logs_template_has_proper_code_blocks():
    ...
    assert "````markdown" in content, "Template must contain markdown code blocks with 4 backticks"
